// <auto-generated />
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using RevCookBookASP.Models;

namespace RevCookBookASP.Migrations
{
    [DbContext(typeof(RevCookBookContext))]
    [Migration("20190806180938_SeedTest")]
    partial class SeedTest
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "2.1.11-servicing-32099")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("RevCookBookASP.Models.Category", b =>
                {
                    b.Property<int>("CategoryId")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Name");

                    b.HasKey("CategoryId");

                    b.ToTable("Categories");

                    b.HasData(
                        new { CategoryId = 1, Name = "Pieczywo" },
                        new { CategoryId = 2, Name = "Nabiał" }
                    );
                });

            modelBuilder.Entity("RevCookBookASP.Models.Dish", b =>
                {
                    b.Property<int>("DishId")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Name");

                    b.Property<string>("Time");

                    b.HasKey("DishId");

                    b.ToTable("Dishes");

                    b.HasData(
                        new { DishId = 1, Name = "Kanapka z masłem", Time = "2 min" }
                    );
                });

            modelBuilder.Entity("RevCookBookASP.Models.DishIngredient", b =>
                {
                    b.Property<int>("DishId");

                    b.Property<int>("IngredientId");

                    b.Property<int>("Amount");

                    b.HasKey("DishId", "IngredientId");

                    b.HasIndex("IngredientId");

                    b.ToTable("DishIngredients");

                    b.HasData(
                        new { DishId = 1, IngredientId = 1, Amount = 1 },
                        new { DishId = 1, IngredientId = 2, Amount = 1 }
                    );
                });

            modelBuilder.Entity("RevCookBookASP.Models.Ingredient", b =>
                {
                    b.Property<int>("IngredientId")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Name");

                    b.HasKey("IngredientId");

                    b.ToTable("Ingredients");

                    b.HasData(
                        new { IngredientId = 1, Name = "Chleb" },
                        new { IngredientId = 2, Name = "Masło" }
                    );
                });

            modelBuilder.Entity("RevCookBookASP.Models.IngredientCategory", b =>
                {
                    b.Property<int>("CategoryId");

                    b.Property<int>("IngredientId");

                    b.HasKey("CategoryId", "IngredientId");

                    b.HasIndex("IngredientId");

                    b.ToTable("IngredientCategories");

                    b.HasData(
                        new { CategoryId = 1, IngredientId = 1 },
                        new { CategoryId = 1, IngredientId = 2 }
                    );
                });

            modelBuilder.Entity("RevCookBookASP.Models.Language", b =>
                {
                    b.Property<int>("LanguageId")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("LanguageCode");

                    b.HasKey("LanguageId");

                    b.ToTable("Languages");

                    b.HasData(
                        new { LanguageId = 1, LanguageCode = "EN" },
                        new { LanguageId = 2, LanguageCode = "PL" }
                    );
                });

            modelBuilder.Entity("RevCookBookASP.Models.Recipe", b =>
                {
                    b.Property<int>("RecipeId")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("DishId");

                    b.Property<int>("LanguageId");

                    b.Property<string>("Text");

                    b.HasKey("RecipeId");

                    b.HasIndex("DishId");

                    b.HasIndex("LanguageId");

                    b.ToTable("Recipes");

                    b.HasData(
                        new { RecipeId = 1, DishId = 1, LanguageId = 1, Text = "Do a sandwitch" },
                        new { RecipeId = 2, DishId = 1, LanguageId = 2, Text = "Zrób kanapkę" }
                    );
                });

            modelBuilder.Entity("RevCookBookASP.Models.DishIngredient", b =>
                {
                    b.HasOne("RevCookBookASP.Models.Dish", "Dish")
                        .WithMany("Ingredients")
                        .HasForeignKey("DishId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RevCookBookASP.Models.Ingredient", "Ingredient")
                        .WithMany("Dishes")
                        .HasForeignKey("IngredientId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RevCookBookASP.Models.IngredientCategory", b =>
                {
                    b.HasOne("RevCookBookASP.Models.Category", "Category")
                        .WithMany("Ingredients")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RevCookBookASP.Models.Ingredient", "Ingredient")
                        .WithMany("Categories")
                        .HasForeignKey("IngredientId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RevCookBookASP.Models.Recipe", b =>
                {
                    b.HasOne("RevCookBookASP.Models.Dish", "Dish")
                        .WithMany("Recipes")
                        .HasForeignKey("DishId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RevCookBookASP.Models.Language", "Language")
                        .WithMany()
                        .HasForeignKey("LanguageId")
                        .OnDelete(DeleteBehavior.Cascade);
                });
#pragma warning restore 612, 618
        }
    }
}
